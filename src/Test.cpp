...
void TEST_BBM_SIMPLE(Array3d target_)
{
 ...
 // 第三个能不能对submap 做一个对比
    ...
    // 构建插入的内容
    // 31 -20 2.7 原值  12 12 0 0 0 0 搜索成功
    // 但是有些值搜不到
    // 多值测试
    vector<Array2d> temp_insert = BasicFunction::Transform(voxel_filtered_scan, target_);
    ...
    CSM::MatchResult_BBM result = csmer_.BranchAndBoundMatch(make_shared<M2MPGM>(m2mpgm_), voxel_filtered_scan, Array3d(20, -50, 1.2));
...
//-40 -40 2
// -40 -20 2
    //for (double x : {160, 80, 40, 20, 0, -20, -40, -80, -160})
    //{
    //    for (double y : {160, 80, 40, 20, 0, -20, -40, -80, -160})
    //    {
    //        for (double theta : {-3, -2, -1, 0, 1, 2, 3})
    //        {
    //            Array3d target(-40, -40, 2);
    //            cout << "the target is " << target.transpose() << endl;
    //            TEST_BBM_SIMPLE(target);
    //        }
    //    }
    //}
    //cout << count_0 << endl;
    ...
    for (int ScanIdx = 0; ScanIdx < mapbuilder_.AllLaserData.size(); ScanIdx++)
    {
            ...
            abs(new_pose(2) - motion_filter_pose(2)) > mapbuilder_.MapParameter_.miniUpdateDR)
        {
            ...
            else
            {
                ...
                point_pool.insert(point_pool.end(), temp_node_submap.begin(), temp_node_submap.end());
                // 子图插入Node 
...
void TEST_FINERMATCHER()
{
    ...
    pgm_set.back().InsertFirstScan(temp, pixelSize);
    csmer_.GenerateSearchWindow(temp, 0.4, 0.5, pixelSize);


    // 准备构建地图的第二帧
    LaserData temp_2 = BasicFunction::Transform(voxel_filtered_scan, Array3d(10.3451, -2.3789, 0.4441));
    LaserFan temp_insert(temp_2, Array3d(10.3451, -2.3789, 0.4441));
    ...
    cout << "the result is " << pose(0) << "," << pose(1) << "," << pose(2) << endl;
    float matching_score = -1.f;
    HillClimbFiner finermatcher_(pgm_set.back(), csmer_);
    pose = finermatcher_.FinerEstimaiton(voxel_filtered_scan, pose,matching_score);
    cout << "the result is " << pose(0) << "," << pose(1) << "," << pose(2) << endl;
}