...
public:
    ...
    const vector<uint16_t> & GetProbabilityValue(){ return ProbabilityValue;}
    MapLimits GetMapLimits()const {return maplimits_;}
    AlignedBox2i GetAlignedBox()const {return scale_;}
    vector<float> GetProbabilityMap();
    double GetPixelSize()const { return pixelSize_; }
    // the XY is in the local frame
    float GetProbability_LocalCandidates(const Eigen::Array2i & XY_)
    {
        assert(maplimits_.Contain(XY_));
        return 1.f - ValueToCorrespondenceCost[XY_(1)*maplimits_.scaleXY(1)+ XY_(0)];
    }
    float GetProbability_WorldCandidates(const Eigen::Array2i& XY_)
    {
        Eigen::Array2i temp = XY_ - Array2i(scale_.min());
        assert(maplimits_.Contain(temp));
        return 1.f - ValueToCorrespondenceCost[temp(1) * maplimits_.scaleXY(1) +temp(0)];
    }
    float GetProbability_WorldCandidates(const Eigen::Array2d& XY_)
    {
